      ...
      <div className="max-w-md mx-auto space-y-6 pb-20">
        ...
        <div className="bg-zinc-900/50 p-5 rounded-[2.5rem] border border-zinc-800 space-y-4">
          <div className="flex gap-2">
            <input value={newPerfume} onChange={(e) => setNewPerfume(e.target.value)} placeholder="Parfüm ekle..." className="flex-1 bg-black border border-zinc-800 p-3 rounded-xl text-xs outline-none" />
        ...
        <div className="group relative border-2 border-dashed border-zinc-800 rounded-[2.5rem] p-12 text-center hover:border-zinc-500 transition-all overflow-hidden shadow-2xl">
          ...
          <label htmlFor="upload" className="cursor-pointer relative z-10 flex flex-col items-center space-y-3">
            ...
            <span className="text-[10px] text-zinc-500 uppercase font-black tracking-[0.3em] group-hover:text-white transition">{image ? "✓ YÜKLENDİ" : "+ KOMBİN EKLE"}</span>
          </label>
        </div>

        {image && (
          <button onClick={analyze} disabled={loading} className="w-full bg-white text-black font-black py-5 rounded-full active:scale-95 transition text-[11px] tracking-[0.2em] shadow-2xl uppercase">
          ...
          <div className="mt-4 space-y-8 animate-in fade-in slide-in-from-bottom duration-1000">
            ...
            <div className="space-y-6">
              {result.recommendations?.map((rec: any, i: number) => (
                <div key={i} className="bg-zinc-900/30 p-6 rounded-[2.5rem] border border-zinc-900 group relative">
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-2 py-4 border-y border-zinc-900">
                    <div className="flex justify-between items-center"><span className="text-[8px] text-zinc-600 uppercase font-black">Longevity</span><span className="text-[10px] text-white font-mono">{rec.metrics?.longevity}</span></div>
                    <div className="flex justify-between items-center"><span className="text-[8px] text-zinc-600 uppercase font-black">Sillage</span><span className="text-[10px] text-white font-mono">{rec.metrics?.sillage}</span></div>
                  </div>
                  <p className="text-[11px] text-zinc-400 leading-relaxed italic mt-4 border-l-2 border-zinc-800 pl-4">"{rec.logic}"</p>
                  <a href={`https://www.fragrantica.com/search/?query=${encodeURIComponent(rec.perfumeName)}`} target="_blank" className="block text-center text-[8px] text-zinc-600 hover:text-white transition uppercase font-black tracking-widest border border-zinc-900 mt-4 py-2.5 rounded-xl">Full Fragrantica Report →</a>
                </div>
              ))}
            </div>
          </div>
