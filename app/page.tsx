    ...
    <div className="min-h-screen bg-black text-white p-6 font-sans">
      <div className="max-w-md mx-auto space-y-6 pb-20">
        ...
        <div onClick={() => document.getElementById('upload')?.click()} className="group relative border-2 border-dashed border-zinc-800 rounded-[2.5rem] p-12 text-center cursor-pointer overflow-hidden">
          ...
          <span className="relative z-10 text-[10px] font-black text-zinc-500 uppercase">{image ? "✓ YÜKLENDİ" : "+ KOMBİN EKLE"}</span>
        </div>
        {image && <button onClick={analyze} disabled={loading} className="w-full bg-white text-black font-black py-5 rounded-full text-xs uppercase">{loading ? "VALIDATING..." : "AURA PUANINI GÖR"}</button>}
        {result && <div className="mt-8 text-9xl font-black italic text-center border-b border-zinc-900 pb-8">{result.auraScore}</div>}
      </div>
    </div>
  );
}